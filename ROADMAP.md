...
- **v0.2b 待做**：装 puppeteer + 跑 render-snapshot 出 bench.png + agent 看图自动产 plan + 对比 demo plan 的覆盖度
...
**当前阶段**：阶段 4 v0.2a ✅ / 待继续 v0.2b（puppeteer 视觉理解链路） / v0.2c（本地图片）
**已完成节点**：
  - ✅ Mode 4 v0.1（commit 6700431）— DocxXML 直转主路径
  - ✅ Mode 3 app（commit 5facda4）— fragment + marked + 字数时长
  - ✅ Mode 4 v0.2a（待 commit）— plan-driven 容器映射机制 + bench demo plan + 视觉验证
**Mode 4 v0.2a 交付**：transform 加 `data-mode4-as` 处理，bench 用 demo plan 重跑后 `.lede-quote` 升级 callout 视觉确认
**阶段 2「Skill 骨架」状态**：未开始（每个 mode 自带 SKILL.md，但跨模式总入口 `skill/SKILL.md` 还没建）
**下一步默认**：按 ROADMAP 阶段顺序推 v0.2b / v0.2c / v0.3 / 阶段 5 / 阶段 6
