...
- 用 `samples/context-tower-9layers/` 跑通 mode 3 端到端
...
- **v0.5c** 装饰块截图兜底——puppeteer 区块 → +media-insert；适合"嵌套 div 套娃 + 复杂渐变背景"场景
- **v0.5d** 全自建 IR fallback——DocxXML 长度上限 / 服务端解析失败时的最后退路；优先级低，等真实失败 sample 出现再做
...
- GH Pages 部署后浏览器质检（commit 7c8ed17 push 后实测）：
  ...
  - ✅ `#app=<base64-utf8-json>` → payload 重建：hero title 改"上下文管理九层塔 · app 态演示"、lede 改"app 态 · agent 已投递 **8** 张图..."、document.title 同步、gallery 用 payload alt 重命名（如"01 / 封面：上下文管理九层塔"）
  - ⏸ 拖排序 / regen 复制 / zip 下载等交互未在 MCP 环境手测（需用户实操），代码路径完整在 app.mjs，本地 dry-run 可验证
...
**当前阶段**：阶段 2a ✅ / Mode 4 v0.2c ✅ / v0.3 ✅ / v0.4 ✅ / v0.5a/b ✅ / 阶段 5 ✅ MVP / **下一步 = 阶段 2b Mode 03 skill 化 + 跨模式 SKILL.md** / Mode 4 v0.5c/d 低优
**已完成节点**（本轮 session）：
  ...
  - ✅ Mode 4 v0.2c 本地/base64 图片（commit 48dc451）— `upload-media.mjs` 两步管线 + sample v0.2c-images-test.html 跑通 `LvijdX2jyolIZOxTYnzcZHVHnig`，**uploaded=2/failed=0**，浏览器质检本地+base64 图位置正确
  - ✅ Mode 4 v0.3（commit 0baf858）— 6 个探针文档 + 3 项 finding 固化 docs/04 §7：ol>callout 服务端吞 + callout 内多 span 粘连用 joinWith 修 + v0.1 Mermaid 内联 DSL 已足够稳
  ...
  - ✅ Mode 4 v0.5b（commit 8fe990b）— `<span style="color">` → DocxXML span text-color/background-color，TUjZdF39LoaYjgxk2sQcBCYLnX4 浏览器质检 6 case 全过
  - ✅ 阶段 5 Mode 2 app 态（待 commit）— fragment 协议 + app.mjs/app.css/make-fragment.mjs + 拖排序 + 单图重生（复制 prompt） + zip 下载
**仍待**（按**实际执行顺序**，已重排）：
  - **阶段 2b — Mode 03 skill 化 + 跨模式 `skill/SKILL.md` 总入口（next）**
  - v0.5c — Mode 4 装饰块截图兜底（低优）
  - v0.5d — Mode 4 全自建 IR fallback（低优）
  - v0.3 — Mode 4 画板验证兜底 + **callout 在 ol/list 上下文的视觉规则探针**（v0.2b finding）
  - v0.4 — Mode 4 局部精修工程化（`docs +update` 8 种指令的 helper）
  - v0.5 — Mode 4 高保真样式 + emoji 白名单专项 + 截图兜底
  - 阶段 5 — Mode 2 app 态（fragment 解析 / 单图重生 / zip 打包）
  - 阶段 2b — Mode 03 skill 化 + 跨模式 `skill/SKILL.md` 总入口
