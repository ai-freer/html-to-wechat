...
- **P**：保留 v0.1 DocxXML 主干。HTML 源里非远程 URL 的 `<img>`（本地路径 / base64 / inline SVG）在 v0.1 是占位文本，v0.2 改为实际上传 + 替换
- **D**：HTML 改造阶段加一遍 `<img>` 扫描——base64 解码到 tmp 文件、本地路径直用、inline SVG 用 puppeteer rasterize；产出图片清单 → `lark-cli docs +media-insert --doc <id> --file <path>` 批量插入。或在 DocxXML 阶段先用占位 + create 后用 +media-insert 替换占位
- **C**：sample（含 base64 图的 context-tower-9layers）跑出来图片正确显示，本地 / base64 / SVG 三种来源都能 cover
- **A**：记录 SVG 转 PNG 的清晰度参数；评估是否要把 base64 图片 inline 到 DocxXML（如果 DocxXML 长度上限允许）
...
**当前阶段**：阶段 3 — Mode 4 v0.1 ✅ 主路径完成 / Mode 3 app 待启动
**当前 PDCA 步骤**：Mode 4 v0.1 进入 A 阶段（总结已写），待 commit；下一节点取决于用户选择
**Mode 4 v0.1 交付**：6 个 skill 文件（`skill/modes/04-feishu/`）+ bench sample 实测成功 + 视觉验证通过；详见上方 ✅ 块
**待用户选择的下一步**：
  1. commit 当前 mode 4 v0.1 工作（推荐先做这个，把成果固化）
  2. Mode 3 app 化（阶段 3 剩余的并行任务）
  3. 跳到阶段 4 v0.2（视觉理解 plan + 本地图片接入）
  4. 跳到阶段 5（Mode 2 app 态）
**阶段 2「Skill 骨架」状态**：未开始（Mode 4 v0.1 直接做了 self-contained 的 SKILL.md + scripts/，但跨模式总入口 `skill/SKILL.md` 还没建）
