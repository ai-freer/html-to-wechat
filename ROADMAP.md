...
**当前阶段**：阶段 6 landing 重制 ✅ / 阶段 4 v0.2b-v0.5 待续 / 阶段 5 Mode 2 app 未启动
**已完成节点**（本轮 session）：
  ...
  - ✅ 阶段 6 landing 重制（待 commit）— 主页改 4-mode 2x2 网格，加飞书卡片
**仍待**（按 ROADMAP 顺序）：
  - v0.2b — puppeteer 截图 + agent 看图自动产 plan（需装 puppeteer ~150MB）
  - v0.2c — 本地/base64 图片走 `+media-insert`
  - v0.3 — 画板验证兜底（需含 mermaid 的 sample）
