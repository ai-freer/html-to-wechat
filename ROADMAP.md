...
**当前阶段**：阶段 2a ✅ / Mode 4 v0.2c ✅ / v0.3 ✅ / v0.4 ✅ / v0.5a/b ✅ / **下一步 = Mode 4 v0.5c 装饰块截图兜底 或 阶段 5 Mode 2 app** / v0.5d 低优 / 阶段 2b 未启动
**已完成节点**（本轮 session）：
  ...
  - ✅ Mode 4 v0.5b（待 commit）— `<span style="color">` → DocxXML span text-color/background-color，TUjZdF39LoaYjgxk2sQcBCYLnX4 浏览器质检 6 case 全过
**仍待**（按**实际执行顺序**，已重排）：
  - **v0.5c — Mode 4 装饰块截图兜底（puppeteer 区块 → +media-insert）** 或可直接跳到阶段 5
  - v0.5d — Mode 4 全自建 IR fallback（最后退路，低优）
