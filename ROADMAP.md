...
- **4 mode skill 全部独立完整**：01-richtext / 02-images / 03-script / 04-feishu，每个自带 SKILL.md + scripts/
- **文档审计缺口已补**：docs/02 已重写（删 satori 规划期方案）+ docs/01/03/04 补 X-A/X-C 范式段 + README 仓库结构 / 状态表 / 演进史更新 + skill/SKILL.md 总入口表更新
...
**已完成节点**（按 commit 时间序）：
  ...
  - ✅ ROADMAP 收尾 + Mode 2 alt bug 修 + Mode 4 v0.5c/d（commit b886b18）
  - ✅ samples 内部化 + .gitignore（commit 4d70fe9）— untrack 25 个内容样本，留 probe/ 公开测试 fixture
  - ✅ 审计大补全 — 4 mode skill 全独立 + docs 全准确（commit 5163ee3）
  - ✅ **PRD A + PRD B 实战 PDCA**（本轮待 commit）：
    - **PRD A**（出行攻略 1.8 MB / 25 article / 19 SVG / 11 base64）→ DocxXML 7.8 KB 压缩 230×，5 个 `.assumption` 转 callout 📌 (joinWith " — ")，6 张图 uploaded=6/0，doc `OKtkdBrkSoA4x4xcJ4pcPXrsnKh` 全质检通过
    - **PRD B**（行程流程 5.6 MB / 13 mermaid DSL / 14 base64）→ DocxXML 18 KB 压缩 306×；**关键 bug 发现 + 修复：mermaid `\n` 被 escapeText collapse**（新增 `escapeXmlPreserveLines()`，whiteboard case 改用它）；修复后 `new_blocks 5 → 13`，13 个 mermaid 画板全部正确渲染（蓝边方框 + 黄色菱形判断 + 标签箭头），doc `QmTWdR6deoOs44x0i73cUo5unqg`
    - **v0.5d 反向证伪**：PRD B 5.6 MB 候选样本 DocxXML 只有 18 KB（base64 走 mediaTasks 不进体 + mermaid DSL 紧凑），**warning 不触发**——v0.5d 实现层可继续 defer 等到真实 4xx 失败 case
    - **PRD B 真实质检 finding · phone mockup 满屏宽**：853×1844 高分辨率竖屏手机截图被 `+media-insert` 按图自身分辨率渲染，撑满文档宽。原 HTML CSS max-width 约束在我们 pipeline 丢失，飞书也不知道这是 mockup 该缩小。修复 (`upload-media.mjs`)：加 `maybeResizePortrait()` —— 用 sharp（optionalDependencies）检测 aspect ratio (w/h) < 0.75 且原宽 > 600px → resize 到 400px 宽，保持宽高比；横屏 / 正方形 / 小图 / icon 不动。重跑 PRD B → 9 张 portrait mockup 全部 853×1844→400×865，wide arch diagram 1270×559 保持不动 → 差异化效果生效，doc `CjX4dgUDboIiNtx4GwmcIvQvn1f` 浏览器质检通过
    - **审计后补全 P2**（本轮，7 项）：
      - **lib/render-snapshot.mjs 共享层**抽离：mode 1/2/3/4 各自的 render-snapshot.mjs 改为薄 shim（注入 `loadPuppeteer` lazy loader 让 dynamic import 找 mode 自己的 node_modules），主体 165 行迁移到 `lib/`。第 4 次出现"应抽离"达成
      - **Mode 4 `<dl>/<dt>/<dd>` 原生支持**：dl 加入 PASS_THROUGH，dt 输出 `<p><b>...</b></p>`，dd 输出 `<p>...</p>`；同时 `<small>/sub/sup/mark/abbr/kbd/samp/var/cite` 等不重要 inline tag 加入 PASS_THROUGH 静默穿透。PRD A/B 警告数 31/40 → **0**
      - **Mode 1 复用 measureImageWidths**：mode 1 `index.mjs` 加 measure 步骤，`transform()` 加 `opts.renderedWidths`，imgs 用 srcPrefix→measurement Map 查找（避免 extract_main 之后索引错位），命中后注入 `<img width=N height=M>`。PRD B 实测 10 imgs widths 全部 applied → 184×406 phone / 650×286 arch 透传到公众号 HTML
      - **Mode 4 v0.5d 实现层**：新增 `chunk-doc.mjs` —— `splitDocxXML(docxxml, maxBytes)` 按顶层 block 边界切分（SAX-lite 找 depth=1 关闭点 + 贪心切割，title 保留首块），`chunkedCreateDoc(chunks)` 首块走 `+create`、其余走 `+update --command append --doc-format xml`。`index.mjs` 加 `--chunk-bytes <N>` CLI 选项。实测 PRD B 18KB → 4 chunks (5KB each)，全部 append 成功，doc `GW0hdgMoFoFsGVx0AaGcE6q0nSf` outline 跨 chunks 完整同步
      - PRD B v0.5e 回归（不分块路径）+ probe 公开项核对 + emoji/mode2-alt 收口 ROADMAP 已记
      - **v0.5d 触发条件二级**：除了 200KB warning + 自动 4xx detection 之外，现支持手动 `--chunk-bytes` 触发；未来真出现 +create 4xx length error 时，可在 createDoc 错误路径自动 fallback chunked 模式
    - **v0.5e · puppeteer 实测渲染宽度，把 HTML CSS layout 真正透传到飞书**（上面 portrait 启发式的"对的方向"升级）：原 HTML 用 CSS（`max-width` / grid 比例 / `width:100%` 在 grid 容器里）控制每张图的实际显示宽度，但 cheerio 没有 layout 维度。新增 `measureImageWidths(htmlPath)` 走 puppeteer 跑 layout → 输出每张 img 的真实 px width + srcPrefix。`transform()` 加 `opts.renderedWidths` 入参，把实测宽度按 DOM 序对齐注入 `data-render-width` 属性。`pickImgDimensions()` 优先采用这个属性（priority 0）。`emitImg` 同步在 mediaTask 加 `widthIsRendered` flag。`upload-media.mjs` 的 resize 重构：优先用 `task.width`（实测）作 resize 目标，启发式降级为"实测不可用"时的兜底。重跑 PRD A 实测 8 张 imgs 全部命中：hero phone 853×1844→**272×588**（原 layout 272px），gallery 2 列 phone 853×1844→**262×566**（2-col grid 各占 262px），arch diagram 1270×559→**508×224**（layout 70% 宽），流程图 1706×922→**560×303**，info png 1536×1024→**560×373**。doc `ZhDDdBNFloaosfxfv8HcKeMLnJc` 浏览器质检：每张图的飞书显示宽度跟原 HTML 几乎像素级一致，差异化彻底成立。Learning：跨模式的"看图理解"基础设施现在多了一项——renderedWidths 测量，未来 mode 1（公众号也吃图）/ mode 2（图集已有自己机制）都可考虑复用
  - ✅ **审计后大补全（本轮待 commit）**：
    - 建 **skill/modes/02-images/** 完整 skill 包（SKILL.md 详写 6 步 agent contract + scripts/render-snapshot + make-fragment + payload-example + package.json），fragment smoke test 1338 chars 通过
    - Mode 03 skill 补 **render-snapshot.mjs**（X-C 接口预留）+ SKILL.md 明写"X-C 通常退化为不需要"+ 多 article 场景的 plan 兜底
    - **重写 docs/02**：删 satori/Canvas 模板规划期方案，重写为 agent 主导 + gpt-image-2 + fragment 投递的真实 MVP；含 X-C/X-A 范式、6 步管线、prompt 工程、payload schema、风险约束
    - docs/01/03/04 顶部各加 X-C/X-A 范式归位段（指向 skill/SKILL.md 总论）
    - **README 大改**：状态表加"实现状态"列、项目形态改为"1 总入口 + 4 模式 skill + 静态前端"清晰表述、仓库结构画得完整（不再有不存在的 02-images/scripts/）、删过时"当前测试输入"、加测试样本指引、加项目演进史
    - skill/SKILL.md 总入口：mode 02 状态行改正、安装/调用示例 4 mode 都给具体 CLI、跨模式工具说明同步更新
    - 建 **samples/context-tower-9layers/payload.json**（真实图集 fragment 范本，7067 chars URL）

**仍待**（明确 defer，等真触发条件出现）：
  - v0.5d 自动 fallback — 已有手动 `--chunk-bytes <N>` 触发路径（实测 PRD B 4 chunks 全跑通）；自动检测 `+create` 4xx length-related stderr 并自动 fallback 到 chunked 模式，等真实失败 sample 出现再实现错误模式匹配
