...
**底层产物**：
- N 张 PNG（每张一次 gpt-image-2 调用产出，commit 到仓库 `samples/<name>/images/` 或图床）
...
input.html
   ...
   │
   ├─ ③ 把图放公网（远程 URL 必须公网可读）
   │     选 1：commit 进 samples/<name>/images/ → raw.githubusercontent.com URL
   │     选 2：imgur / sm.ms / 用户自有图床
   │
...
- ✅ **真实图集 demo**：`samples/context-tower-9layers/`（8 张 AI 卡 + caption + prompts + plan + learnings + style_sheet）
