...
| 4. HTML → 飞书云文档（Block 直转） | 飞书云文档 / 团队协同 | ❌ Skill-only（需飞书 CLI + UAT） | [docs/04-html-to-feishu.md](docs/04-html-to-feishu.md) |

四种模式共享同一个输入（标准 HTML 文件），但产出形态完全不同。整体推进计划见 [ROADMAP.md](ROADMAP.md)。
...
两条路径互不强耦合：模式 1 的页面可以脱离 Skill 独立用；模式 2、3 需要 LLM/生图/TTS 参与，必须由 Skill 驱动，最后产物在前端做交付；模式 4 是 100% Skill-only（需要飞书 CLI 与 OAuth token），没有"纯前端入口"。
...
│   ├── index.html                  # Landing：项目门面 + 四模式入口卡
...
└── skill/                          # Skill 包，可独立安装
    ...
    └── modes/
        ...
        ├── 02-images/scripts/
        ├── 03-script/scripts/
        └── 04-feishu/scripts/       # 模式 4：飞书 CLI 调用 + Block emit
```
