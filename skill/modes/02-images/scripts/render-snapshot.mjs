// Mode 2 · 视觉理解前置 — puppeteer 截图（agent 拆卡片前必做）
//
// **同步来源**：本文件主体逻辑与 skill/modes/04-feishu/scripts/render-snapshot.mjs 等价。
// 两 mode 的视觉理解截图共用一套 viewport（1280×800 dsf=2 fullPage）和 selector 区块截图。
// 第三次出现时（mode 1/2/4）应该正式抽到 lib/ 共用。
//
// 见 docs/02-html-to-images.md §4.0（视觉理解强制约束）
