...
1. **图床**（imgur / sm.ms / Cloudflare R2 / 用户自有 CDN）：上传后拿 URL — 最常用
2. **`raw.githubusercontent.com`**：commit 到自己的 fork / 独立仓库（注意：本 html-to-wechat 仓库 `samples/` 已 `.gitignore` 内部用，**不能**作为公开图源）
