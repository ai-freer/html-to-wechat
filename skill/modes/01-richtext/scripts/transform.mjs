// HTML → 公众号富文本 · 1A 引擎（pure function）—— SKILL 端口
//
// 范式 X-A/X-C 的执行层：transform(html, plan) → { html, text, stats, warnings }
//
// "1A 是纯函数" — 无 IO、无 LLM、无 UI 副作用。所有"长尾"决策都被 plan 吸收。
// 两个入口共享同一份主体代码：
//   - web/richtext/transform.mjs（网页 UI）：juice 走 esm.sh CDN，浏览器原生 DOMParser
//   - skill/modes/01-richtext/scripts/transform.mjs（**本文件**）：juice 走 npm，DOMParser 由 jsdom 注入
//
// **同步约定**：本文件从 web/richtext/transform.mjs 移植；除顶部 imports 和 DOMParser 注入两处外，
// 主体逻辑必须保持 byte-identical。后续：修 bug 两处一起改；累积差异大后抽到 shared lib/。
// 见 docs/01-html-to-wechat-html.md §2 / §4，ROADMAP 阶段 2a「Act」段。

import juice from 'juice';
import { JSDOM } from 'jsdom';
import { DEFAULT_PLAN, mergePlan } from './default-plan.mjs';

// 注入 DOMParser 到模块作用域（jsdom 通过 window 暴露浏览器兼容的 DOMParser）
